...
-- --- Application roles -------------------------------------------------------
-- Dev passwords only (docker-compose). In AWS the real credentials come from
-- Secrets Manager: app_rw is reached via `SET ROLE` after the app connects as
-- master (so its password never matters), while data_rw is authenticated
-- directly by the data repo through RDS Proxy — its password is generated by
-- Terraform (rds module) and synced onto this role out-of-band via the ops
-- Lambda `set-role-password` action (see docs/RUNBOOK.md). The `if not exists`
-- guard below means re-running bootstrap never clobbers that managed password.
-- app_rw is NOT BYPASSRLS — that is the hard guarantee of tenant isolation.
do $$
begin
