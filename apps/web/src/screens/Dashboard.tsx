  ...
  return (
    <>
      ...
      <QueryBoundary query={query}>
        {(d) => {
          const shownSites =
            scope.type === 'all' ? d.sites : d.sites.filter((s) => s.siteId === scope.siteId);
          ...
          const site =
            scope.type === 'site' ? d.sites.find((s) => s.siteId === scope.siteId) : undefined;
          const kpi = site
            ? {
              ...
              <div className="mb-[18px] flex flex-wrap gap-3.5">
                <Card className="flex-[2.4_1_250px] p-4">
                  <div className="mb-2 flex items-center gap-2 text-[12.5px] font-semibold text-fg-muted">
                    ...
                    <InfoDot title="Recettes du jour">
                      Chiffre d'affaires encaissé aujourd'hui sur le périmètre sélectionné (toutes
                      machines, tous moyens de paiement). Le badge compare au même total d'hier ; la
                      courbe retrace les 9 derniers jours de recettes.
                    </InfoDot>
                  </div>
                ...
                <KpiSmall
                  ...
                  value={
                    <>
                      {kpi.machinesActive}
                      <span className="text-base font-semibold text-fg-subtle">
                        {' '}
                        / {kpi.machinesTotal}
                      </span>
                    </>
                ...
                >
                  ...
                  <div className="mt-1.5 text-[11.5px] text-fg-subtle">
                    {Math.round((kpi.machinesActive / kpi.machinesTotal) * 100)} % du parc en
                    service
                  </div>
                </KpiSmall>
                ...
                >
                  <Pill tone="danger" className="mt-3">
                    {site
                      ? kpi.machinesOutOfService > 0
                        ? 'sur ce site'
                        : 'aucun sur ce site'
                      : `${kpi.sitesWithOosCount} sites concernés`}
                  </Pill>
                </KpiSmall>
                ...
                >
                  <div className="mt-3 text-[11.5px] text-fg-subtle">
                    {kpi.energyVsRefPct < 0
                      ? 'Sous la référence du parc'
                      ...
                      {shownSites.map((s) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="w-[30px] text-right text-[11.5px] tabular-nums text-fg-muted">
                              {s.occupancyPct}%
                            </span>
                          </div>
                          <div
                          ...
                          <div className="flex items-center gap-2">
                            <ProgressBar
                              value={s.benchmarkPercentile}
                              tone={benchTone(s.benchmarkPercentile)}
                              className="flex-1"
                            />
                    ...
                    {shownAlerts.length === 0 ? (
                      ...
                      shownAlerts.map((a) => (
                        <AlertRow
                          key={a.id}
                          alert={a}
                          onOpen={() => navigate({ to: '/notifications' })}
                        />
                      ))
                    )}
                  </SectionCard>
...
function KpiSmall({
  ...
  return (
    <Card className="flex-[1_1_158px] p-4">
      ...
      <div className={cn('text-[26px] font-bold tabular-nums tracking-[-0.5px]', valueClass)}>
        {value}
      </div>
      {children}
    </Card>
  ...
  return (
    <div className="grid grid-cols-2 gap-3 p-[18px]">
      <div className="rounded-[10px] border border-border bg-surface-2 p-3.5">
        <div className="mb-1 text-[11px] font-semibold uppercase tracking-[0.3px] text-fg-subtle">
          CA du jour
        </div>
        <div className="text-[22px] font-bold tabular-nums tracking-[-0.5px]">
          {money0(s.revenue)}
        </div>
        <div
          className={cn(
            'mt-1 text-[11.5px] font-semibold',
            s.revenueDeltaPct < 0 ? 'text-danger' : 'text-ok',
          )}
        >
      ...
      <div className="rounded-[10px] border border-border bg-surface-2 p-3.5">
        ...
        <div
          className={cn(
            ...
            s.openAlerts === 0 ? 'text-ok' : s.openAlerts >= 3 ? 'text-danger' : 'text-warn',
          )}
        >
          {s.openAlerts}
        </div>
...
function AlertRow({ alert, onOpen }: { alert: DashboardAlert; onOpen?: () => void }) {
  ...
  return (
    <div
      onClick={onOpen}
      className="flex cursor-pointer gap-[11px] border-b border-border px-[17px] py-3 last:border-b-0 hover:bg-surface-2"
    >
      <div
        className={cn(
